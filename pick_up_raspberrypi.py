...
class PoseSubscriberClient(Node):
    ...
    def pose_callback(self, msg):
        ...
        try:    
            for j in range(3):
                for i in range(4):
                    #Subscribe pose first detected raspberry pi (z-component)!!!!
                    x_matrix= -24 + (62*i) #hard code
                    y_matrix= -171 - (93*j) #hard code
                    z_matrix= 184 #subscribe z-component
                    x = msg.x
                    y = msg.y
                    z = msg.z

                    #Move robot arm to initial robot state
                    self.move_pose_client.send_request(0.07, 0.19, 1.45, 3.16, -1.30, 1.84)

                    #Move robot arm to Etikettendrucker
                    self.move_pose_client.send_request(0.07, 0.57, 0.99, 3.18, -0.46, 1.81)

                    #Move robot arm over box
                    self.move_pose_client.send_request(-1.36, 0.19, 1.43, 3.12, -1.2, 1.73)

                    #Move robot to Raspberry PI Pose
                    self.send_request(x_matrix, y_matrix, z_matrix, 80)
                    ...
                    #Move robot arm to goal position (up)
                    self.move_pose_client.send_request(0.58, 0.54, 1.62, 3.18, -1.11, 2.33)

                    #Move robot arm to goal position (down)
                    self.move_pose_client.send_request(0.58, 1.06, 1.50, 3.23, -0.42, 2.28)
            ...
            self.move_pose_client.send_request(-1.61, -0.02, 1.52, 0.02, 1.57, 1.60)

            #Move to goal Pose (down)
            self.move_pose_client.send_request(-1.60, 1.60, 1.33, -0.04, -0.24, 1.64)
            ...
            self.move_pose_client.send_request(0.15, -0.02, 1.52, 0.02, 1.57, 1.60)
            """
    ...
    def send_request(self, x, y, z, velocity):
        ...
        self.req.pose = array('f', [float(x), float(y), float(z),3.1 , 0.01, 0.06])
        self.get_logger().info(f'Sending pose: [{x}, {y}, {z},3.1 , 0.01, 0.06')
